...
interface DeleteExamProps {
  ...
  open: boolean;
  onClose: () => void;
  onDeleteSuccess: (examId: number) => void;  // הוספת prop לעדכון הסטייט בממשק
}

const DeleteExam = ({ examId, open, onClose, onDeleteSuccess }: DeleteExamProps) => {

  const handleDelete = async () => {
    try {
      await axios.delete(`https://localhost:7158/api/Exam/${examId}`);
      onDeleteSuccess(examId);  // עדכון הסטייט בממשק הראשי
